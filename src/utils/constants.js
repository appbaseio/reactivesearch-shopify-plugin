const appbasePrefs = {
    name: 'Search geo1-demo-app',
    description: '',
    pipeline: 'geo1-demo-app',
    id: '2f6ad2d5-f627-4151-bb64-13eb2b0ae3e6',
    pageSettings: {
        currentPage: 'home',
        pages: {
            home: {
                componentSettings: {
                    search: {
                        componentType: 'SEARCHBOX',
                        customMessages: {
                            noResults:
                                'No suggestions found for <mark>[term]</mark>',
                        },
                        searchButton: {
                            icon: '',
                            text: 'Click here to search',
                        },
                        redirectUrlText: 'View Product',
                        redirectUrlIcon: '',
                        fields: {
                            title: 'place',
                            description: 'magnitude',
                            price: 'year',
                            priceUnit: null,
                            image: '',
                            handle: '',
                        },
                        rsConfig: {
                            autosuggest: true,
                            enablePopularSuggestions: false,
                            enableRecentSearches: false,
                            highlight: false,
                            showVoiceSearch: true,
                            componentType: 'SEARCHBOX',
                        },
                    },
                    result: {
                        componentType: 'REACTIVELIST',
                        fields: {
                            title: 'place',
                            description: 'magnitude',
                            price: 'year',
                            priceUnit: null,
                            image: '',
                            handle: '',
                        },
                        customMessages: {
                            resultStats: '[count] products found in [time] ms',
                            noResults: 'No Results Found!',
                        },
                        rsConfig: {
                            pagination: false,
                            infiniteScroll: true,
                            componentType: 'REACTIVE_MAP',
                        },
                        sortOptionSelector: [],
                        resultHighlight: false,
                        layout: 'grid',
                        viewSwitcher: true,
                        mapLayout: 'map',
                        locationDataField: 'location',
                        mapComponent: 'openStreetMap',
                        defaultZoom: 5,
                        showSearchAsMove: true,
                        showMarkerClusters: false,
                        mapsAPIkey: 'AIzaSyA9JzjtHeXg_C_hh_GdTBdLxREWdj3nsOU',
                    },
                    productType: {
                        name: 'productType',
                        enabled: false,
                        isCollapsible: true,
                        customMessages: {
                            loading: 'Fetching Options',
                            noResults: 'No items Found',
                        },
                        rsConfig: {
                            title: null,
                            filterType: 'list',
                            queryFormat: 'or',
                            sortBy: 'count',
                            filterLabel: null,
                            showCount: true,
                            showCheckbox: true,
                            showSearch: true,
                            showMissing: false,
                            missingLabel: null,
                            multiSelect: false,
                            selectAllLabel: null,
                            componentType: 'MULTILIST',
                        },
                        componentType: 'MULTILIST',
                        facetType: 'static',
                    },
                    collection: {
                        name: 'collection',
                        enabled: false,
                        isCollapsible: true,
                        customMessages: {
                            loading: 'Fetching Options',
                            noResults: 'No items Found',
                        },
                        rsConfig: {
                            title: null,
                            filterType: 'list',
                            queryFormat: 'or',
                            sortBy: 'count',
                            filterLabel: null,
                            showCount: true,
                            showCheckbox: true,
                            showSearch: true,
                            showMissing: false,
                            missingLabel: null,
                            multiSelect: false,
                            selectAllLabel: null,
                            componentType: 'MULTILIST',
                        },
                        componentType: 'MULTILIST',
                        facetType: 'static',
                    },
                    color: {
                        name: 'color',
                        enabled: false,
                        isCollapsible: true,
                        customMessages: {
                            loading: 'Fetching Options',
                            noResults: 'No items Found',
                        },
                        rsConfig: {
                            title: null,
                            dataField: null,
                            filterType: 'list',
                            queryFormat: 'or',
                            sortBy: 'count',
                            filterLabel: null,
                            showCount: true,
                            showCheckbox: true,
                            showSearch: true,
                            showMissing: false,
                            missingLabel: null,
                            multiSelect: false,
                            selectAllLabel: null,
                            componentType: 'MULTILIST',
                        },
                        componentType: 'MULTILIST',
                        facetType: 'static',
                    },
                    size: {
                        name: 'size',
                        enabled: false,
                        isCollapsible: true,
                        customMessages: {
                            loading: 'Fetching Options',
                            noResults: 'No items Found',
                        },
                        rsConfig: {
                            title: null,
                            dataField: null,
                            filterType: 'list',
                            queryFormat: 'or',
                            sortBy: 'count',
                            filterLabel: null,
                            showCount: true,
                            showCheckbox: true,
                            showSearch: true,
                            showMissing: false,
                            missingLabel: null,
                            multiSelect: false,
                            selectAllLabel: null,
                            componentType: 'MULTILIST',
                            showHistogram: false,
                            startValue: null,
                            endValue: null,
                            startLabel: null,
                            endLabel: null,
                            calendarInterval: null,
                        },
                        componentType: 'MULTILIST',
                        facetType: 'static',
                    },
                    price: {
                        name: 'price',
                        enabled: false,
                        isCollapsible: true,
                        customMessages: {
                            loading: 'Fetching Options',
                            noResults: 'No items Found',
                        },
                        rsConfig: {
                            title: null,
                            dataField: null,
                            startValue: null,
                            endValue: null,
                            startLabel: null,
                            endLabel: null,
                            showHistogram: false,
                            componentType: 'DYNAMICRANGESLIDER',
                        },
                        componentType: 'DYNAMICRANGESLIDER',
                        facetType: 'static',
                    },
                },
            },
        },
        fields: {
            description: 'magnitude',
            handle: '',
            image: '',
            price: 'year',
            priceUnit: null,
            title: 'place',
        },
    },
    themeSettings: {
        type: 'geo',
        customCss: '',
        rsConfig: {
            colors: {
                primaryColor: '#0B6AFF',
                primaryTextColor: '#fff',
                textColor: '#424242',
                titleColor: '#424242',
            },
            typography: {
                fontFamily: 'default',
            },
        },
    },
    globalSettings: {
        currency: 'USD',
        showSelectedFilters: true,
        meta: {
            branding: {
                logoUrl: '',
                logoWidth: 20,
                logoAlignment: 'left',
            },
            deploySettings: {
                versionId: 'R9nyNxTPgupdgEazzo3ULVjhrkAGRQo8',
            },
        },
    },
    exportSettings: {
        exportAs: 'embed',
        credentials: '03f653924eff:122f0386-6b34-4da7-a7db-4f9633f26b85',
        openAsPage: false,
        type: 'other',
    },
    resultSettings: {
        fields: {
            title: 'place',
            description: 'magnitude',
            price: 'year',
            priceUnit: null,
            image: '',
            handle: '',
        },
        customMessages: {
            resultStats: '[count] products found in [time] ms',
            noResults: 'No Results Found!',
        },
        rsConfig: {
            pagination: false,
            infiniteScroll: true,
            componentType: 'REACTIVE_MAP',
        },
        sortOptionSelector: [],
        resultHighlight: false,
        layout: 'grid',
        viewSwitcher: true,
        mapLayout: 'map',
        locationDataField: 'location',
        mapComponent: 'openStreetMap',
        defaultZoom: 5,
        showSearchAsMove: true,
        showMarkerClusters: false,
        mapsAPIkey: 'AIzaSyA9JzjtHeXg_C_hh_GdTBdLxREWdj3nsOU',
    },
    searchSettings: {
        customMessages: {
            noResults: 'No suggestions found for <mark>[term]</mark>',
        },
        searchButton: {
            icon: '',
            text: 'Click here to search',
        },
        redirectUrlText: 'View Product',
        redirectUrlIcon: '',
        fields: {
            title: 'place',
            description: 'magnitude',
            price: 'year',
            priceUnit: null,
            image: '',
            handle: '',
        },
        rsConfig: {
            autosuggest: true,
            enablePopularSuggestions: false,
            enableRecentSearches: false,
            highlight: false,
            showVoiceSearch: true,
            componentType: 'SEARCHBOX',
        },
    },
    facetSettings: {
        staticFacets: [
            {
                name: 'productType',
                enabled: false,
                isCollapsible: true,
                customMessages: {
                    loading: 'Fetching Options',
                    noResults: 'No items Found',
                },
                rsConfig: {
                    title: null,
                    filterType: 'list',
                    queryFormat: 'or',
                    sortBy: 'count',
                    filterLabel: null,
                    showCount: true,
                    showCheckbox: true,
                    showSearch: true,
                    showMissing: false,
                    missingLabel: null,
                    multiSelect: false,
                    selectAllLabel: null,
                    componentType: 'MULTILIST',
                },
            },
            {
                name: 'collection',
                enabled: false,
                isCollapsible: true,
                customMessages: {
                    loading: 'Fetching Options',
                    noResults: 'No items Found',
                },
                rsConfig: {
                    title: null,
                    filterType: 'list',
                    queryFormat: 'or',
                    sortBy: 'count',
                    filterLabel: null,
                    showCount: true,
                    showCheckbox: true,
                    showSearch: true,
                    showMissing: false,
                    missingLabel: null,
                    multiSelect: false,
                    selectAllLabel: null,
                    componentType: 'MULTILIST',
                },
            },
            {
                name: 'color',
                enabled: false,
                isCollapsible: true,
                customMessages: {
                    loading: 'Fetching Options',
                    noResults: 'No items Found',
                },
                rsConfig: {
                    title: null,
                    dataField: null,
                    filterType: 'list',
                    queryFormat: 'or',
                    sortBy: 'count',
                    filterLabel: null,
                    showCount: true,
                    showCheckbox: true,
                    showSearch: true,
                    showMissing: false,
                    missingLabel: null,
                    multiSelect: false,
                    selectAllLabel: null,
                    componentType: 'MULTILIST',
                },
            },
            {
                name: 'size',
                enabled: false,
                isCollapsible: true,
                customMessages: {
                    loading: 'Fetching Options',
                    noResults: 'No items Found',
                },
                rsConfig: {
                    title: null,
                    dataField: null,
                    filterType: 'list',
                    queryFormat: 'or',
                    sortBy: 'count',
                    filterLabel: null,
                    showCount: true,
                    showCheckbox: true,
                    showSearch: true,
                    showMissing: false,
                    missingLabel: null,
                    multiSelect: false,
                    selectAllLabel: null,
                    componentType: 'MULTILIST',
                    showHistogram: false,
                    startValue: null,
                    endValue: null,
                    startLabel: null,
                    endLabel: null,
                    calendarInterval: null,
                },
            },
            {
                name: 'price',
                enabled: false,
                isCollapsible: true,
                customMessages: {
                    loading: 'Fetching Options',
                    noResults: 'No items Found',
                },
                rsConfig: {
                    title: null,
                    dataField: null,
                    startValue: null,
                    endValue: null,
                    startLabel: null,
                    endLabel: null,
                    showHistogram: false,
                    componentType: 'DYNAMICRANGESLIDER',
                },
            },
        ],
        dynamicFacets: [],
    },
    syncSettings: null,
    appbaseSettings: {
        index: 'geo1-demo-app',
        credentials: '03f653924eff:122f0386-6b34-4da7-a7db-4f9633f26b85',
        url: 'https://appbase-demo-ansible-abxiydt-arc.searchbase.io',
    },
};
export default JSON.stringify(appbasePrefs);
